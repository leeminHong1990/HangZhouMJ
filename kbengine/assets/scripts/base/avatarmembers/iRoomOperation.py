...
class iRoomOperation(object):
	...
	def createRoom(self, create_dict):
		...
		def callback(content):
			...
			try:
				...
				card_cost, diamond_cost = switch.calc_cost(game_round, {"game_mode":game_mode, "pay_mode":pay_mode, "game_max_lose":game_max_lose})
			...
			if room.room_type == const.AGENT_ROOM:
				if room.pay_mode == const.AGENT_PAY_MODE:
					...
					if switch.DEBUG_BASE:
						...
						utility.update_card_diamond(self.accountName, -card_cost, -diamond_cost, callback, "HangZhou RoomID:{}".format(room.roomID))  # reason 必须为英文
	...
	def startGame(self, dealer_idx, tile_list, wreaths_list, kingTiles, prevailing_wind, player_wind_list, diceList, swap_list, cur_dealer_mul):
		if getattr(self, 'client', None):
			self.client.startGame(dealer_idx, tile_list, wreaths_list, kingTiles, prevailing_wind, player_wind_list, diceList, swap_list, cur_dealer_mul)
