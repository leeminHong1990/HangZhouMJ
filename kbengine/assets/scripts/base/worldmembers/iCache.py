...
class iCache:
	def __init__(self):
		...
		self.cachedInfos = {}
		# SELECT sm_name,sm_wealth FROM switch.DB_NAME.tbl_Avatar;
		...
		KBEngine.executeRawDatabaseCommand("SELECT `sm_uuid`,`sm_name`, `sm_characterNum`, \
			`sm_gender`, `sm_userId`, `id`, `sm_curPower`, `sm_blockFriends` FROM "+switch.DB_NAME+".tbl_Avatar;", queryDatabaseCallBack)
