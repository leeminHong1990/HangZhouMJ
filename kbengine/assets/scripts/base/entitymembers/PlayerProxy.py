...
class PlayerProxy(object):
	...
	def add_score(self, score):
		if self.owner.round_max_lose > 0 and self.score + score < -self.owner.round_max_lose:
			real_lose = -self.owner.round_max_lose - self.score
			self.score = -self.owner.round_max_lose
			return real_lose
		else:
			self.score += score
			return score
	...
	def concealedKong(self, tile):
		...
		def delay_callback():
			...
			self.owner.waitForOperation(self.idx, const.OP_CONCEALED_KONG, tile, self.idx)
	...
	def continueKong(self, tile):
		...
		# 算分
		self.owner.cal_score(self.idx, self.idx, const.OP_CONTINUE_KONG, 1)
		self.owner.last_player_idx = self.idx
	...
	def kongWreath(self, tile):
		DEBUG_MSG("room:{},curround:{} Player:{} kongWreath:{}".format(self.owner.roomID, self.owner.current_round, self.idx, tile))
		self.tiles.remove(tile)
	...
	def drawTile(self, tile, is_first = False):
		""" 摸牌 """
		DEBUG_MSG("room:{0},curround:{1} Player{2} drawTile: {3}, left = {4}".format(self.owner.roomID, self.owner.current_round, self.idx, tile, len(self.owner.tiles)))
		self.last_draw = tile
		self.tiles.append(tile)
	...
	def count_draw_king(self, tile_list):
		for t in self.owner.kingTiles:
			self.draw_king_times += tile_list.count(t)

	def cutTile(self, tile):
		"""切牌"""
		DEBUG_MSG("room:{},curround:{} Player:{} cutTile:{}".format(self.owner.roomID, self.owner.current_round, self.idx, tile))
		...
		self.owner.broadcastOperation(self.idx, const.OP_CUT, [tile,])

	def discardTile(self, tile = None):
		""" 打牌 """
		if tile is None:
			tile = self.last_draw
		DEBUG_MSG("room:{},curround:{} Player:{} discardTile:{}".format(self.owner.roomID, self.owner.current_round, self.idx, tile))
		self.tiles.remove(tile)
		self.owner.last_player_idx = self.idx
		# 出财神处理
		if tile in self.owner.kingTiles:
			self.owner.discard_king_idx = self.idx
		elif self.owner.discard_king_idx == self.idx:
			self.owner.discard_king_idx = -1
