...
var GameRoomPrepareUI = UIBase.extend({
    ...
    get_share_desc : function () {
        ...
        if(player.curGameRoom.game_mode === 0){
            ...
            share_list.push(player.curGameRoom.maxRound + '局');
            share_list.push('底分' + player.curGameRoom.base_score);
            share_list.push(player.curGameRoom.round_max_lose === 0 ? '无封顶' : player.curGameRoom.round_max_lose + '分封顶');

        } else {
            share_list.push('带入' + player.curGameRoom.game_max_lose);
        }
        // 玩法
        share_list.push(roominfo_list[1][player.curGameRoom.king_mode]);
        share_list.push(roominfo_list[2][player.curGameRoom.begin_dealer_mul - 1]);
        share_list.push(roominfo_list[3][player.curGameRoom.win_mode]);

        if(player.curGameRoom.three_job === 1){
            share_list.push( "三摊承包");
        }
        if(player.curGameRoom.pong_useful === 1){
            share_list.push( "碰算摊");
        }
        if(player.curGameRoom.bao_tou === 1){
	...
	playVoiceAnim:function(serverSitNum, record_time){
		...
		if(serverSitNum < 0){
			...
			player_info_panel = this.gameprepare_panel.getChildByName("player_info_panel" + serverSitNum);
			// player_info_panel = this.gameprepare_panel.getChildByName("player_info_panel" + h1global.player().server2CurSitNum(serverSitNum));
		}
        var talk_img = ccui.ImageView.create();
		if(this.curRound> 0) {
